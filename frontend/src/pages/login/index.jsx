...
export default class Login extends Component {
    ...
    componentDidMount() {
        ...
        if (process.env.NODE_ENV === "development" || process.env.PREVIEW) {
            this.form.setFieldsValue({ username: "admin", password: "123" });
        }
    ...
    handleSubmit = (values) => {
        ...
        this.props.ajax
            .post(`api/system/login`, params, { errorTip: true })
            .then((res) => {
                // debugger
                if (res.status == "success" && res.statusCode == "200") {
                    const { id, account,userName, avatar,createTime, codes, codeMap } =
                        res.result.user;
                    setLoginUser({
                        id, // 必须字段
                        userName,
                    ...
                    if (res.result.user.indexPage) {
                        locationHref(
                            `${preRouter}${res.result.user.indexPage}`
                        );
                    } else {
                        toHome();
                    }
    ...
    // 去注册页
    toRegister = () => {
        locationHref('/front_nft_pc/register')
    }

    render() {
        const { loading, message, isMount, captchaData, showCaptcha } =
            this.state;
