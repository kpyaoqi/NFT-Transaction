...
export default config({
  ...
  async function handleSubmit(values) {
    props.ajax.post(`${CODEMAP.nft_mobile_userchainplat_userDelPrivateAndPublic}?verificationCode=${values.verificationCode}`).then(res => {
  ...
  return (
    ...
    >
      <h3>当前用户手机号：{phone}</h3>
      <h4 style={{color:'red'}}>（注：当前证书删除后将无法恢复，无法正常交易NFT藏品）</h4>
      <Form
        form={form}
