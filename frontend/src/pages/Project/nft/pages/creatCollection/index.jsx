...
export default class nft_creatCollection extends Component {
  ...
  handleCollection(){
    ...
    this.props.ajax.post(`${CODEMAP.nft_mobile_collection_colletionDetail}?nftCollectionId=${id}`).then(res => {
      if (res.status == 'success' && res.statusCode == '200') {
        this.setState({
          listData:res.result,
          imageUrl:STATIC_IMG + res.result.logoPath,
          imageUrl1:STATIC_IMG + res.result.coverPath,
          imageUrl2:STATIC_IMG + res.result.bannerPath,
  ...
  render() {
    ...
    // console.log(111, `${AJAX_PREFIXCONFIG}`)
    // console.log(222, `${CODEMAP.t_attachment_upload}`)
    // console.log(333, `${AJAX_PREFIXCONFIG}${CODEMAP.t_attachment_upload}`)
    const uploadButton = (
