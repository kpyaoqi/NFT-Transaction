...
type NftOrder struct {
	...
	ChainPlatCert *string    `gorm:"column:chainPlatCert;type:json" json:"chainPlatCert"`             // 在不同链平台上链,认证后的数据json
	...
	Num           int64      `gorm:"column:num;type:int;not null" json:"num"`                         // 交易品数量
	...
	PayPlat       *int64     `gorm:"column:payPlat;type:int" json:"payPlat"`                          // 支付平台(0:微信支付,1支付宝...)
	TradeType     int64      `gorm:"column:tradeType;type:int;not null" json:"tradeType"`             // 交易类型(0:充值,1:上架合集,2:下架合集,3:上架商品,4:下架商品,5:交易作品,6:转移作品)
	TxTime        *time.Time `gorm:"column:txTime;type:datetime" json:"txTime"`                       // 交易时间
	TransactionID *string    `gorm:"column:transactionId;type:varchar(100)" json:"transactionId"`     // 支付平台支付订单号
	PayState      int64      `gorm:"column:payState;type:int;not null" json:"payState"`               // 支付状态.0:未支付,1:已支付,2:已取消
